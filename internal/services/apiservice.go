...
import (
	...
	oauth1 "usulroster/internal/auth"
	models "usulroster/ormodel"
)
...
func ProcessAcademicSessionsAPI(domain string, key, secret string, orProcess models.ORProcess) error {
	...
	for hasNext {
		...
		for _, orAcademicSession := range academicSessions {
			...
			if strings.EqualFold(orAcademicSession.Status, models.StatusTypeActive) {
				orAcademicSessionToEdit = append(orAcademicSessionToEdit, orAcademicSession)
			} else if strings.EqualFold(orAcademicSession.Status, models.StatusTypeToBeDeleted) {
	...
	for hasNext {
		...
		for _, org := range orgs {
			...
			if org.OrgType == models.OrgTypeDistrict {
				...
				if strings.EqualFold(org.Status, models.StatusTypeActive) {
					err = orProcess.HandleEditDistrict(org, districtIDs[0])
				} else if strings.EqualFold(org.Status, models.StatusTypeToBeDeleted) {
...
func ProcessCoursesAPI(domain string, key, secret string, orProcess models.ORProcess, districtIDs []string) error {
	...
	for hasNext {
		...
		for _, orCourse := range orCourses {
			...
			if strings.EqualFold(orCourse.Status, models.StatusTypeActive) {
				orCourseToEdit = append(orCourseToEdit, orCourse)
			} else if strings.EqualFold(orCourse.Status, models.StatusTypeToBeDeleted) {
...
func ProcessClassesAPI(domain string, key, secret string, orProcess models.ORProcess, districtIDs []string) error {
	...
	for hasNext {
		...
		for _, orClass := range orClasses {
			...
			if strings.EqualFold(orClass.Status, models.StatusTypeActive) {
				orClassesToEdit = append(orClassesToEdit, orClass)
			} else if strings.EqualFold(orClass.Status, models.StatusTypeToBeDeleted) {
...
func ProcessUsersAPI(domain string, key, secret string, orProcess models.ORProcess, districtIDs []string) error {
	...
	for hasNext {
		...
		for _, orUser := range orUsers {
			...
			if strings.EqualFold(orUser.Status, models.StatusTypeActive) {
				orUsersToEdit = append(orUsersToEdit, orUser)
			} else if strings.EqualFold(orUser.Status, models.StatusTypeToBeDeleted) {
...
func ProcessEnrollmentAPI(domain string, key, secret string, orProcess models.ORProcess, districtIDs []string) error {
	...
	for hasNext {
		...
		for _, orEnrollment := range orEnrollments {
			...
			if strings.EqualFold(orEnrollment.Status, models.StatusTypeActive) {
				orEntrollmentsToEdit = append(orEntrollmentsToEdit, orEnrollment)
			} else if strings.EqualFold(orEnrollment.Status, models.StatusTypeToBeDeleted) {
